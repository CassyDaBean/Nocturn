...
namespace Nocturn.Projectiles.SW
{
    class GigababyHead : ModProjectile
    {
        ...
        {
            DisplayName.SetDefault("Gigaworm Bady");
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            Main.spriteBatch.Draw(Head, projectile.Center - Main.screenPosition + new Vector2(0f, projectile.gfxOffY), new Rectangle(0, 0, Head.Width, num214),
                projectile.GetAlpha(lightColor), projectile.rotation, new Vector2(Head.Width / 2f, Head.Height / 2f), projectile.scale,
        ...
        public override void AI()
        {
            ...
            bool flag64 = projectile.type == mod.ProjectileType("GigababyHead");
            if ((int)Main.time % 120 == 0) projectile.netUpdate = true;
            //Main.NewText(modPlayer.GigaBab);
            ...
            if (!modPlayer.GigaBab) projectile.active = false;
            if (projectile.active)
            {
                modPlayer.GigaBab = true;
            }
                if (player.dead) modPlayer.GigaBab = false;
                if (modPlayer.GigaBab) projectile.timeLeft = 2;
    ...
    public class GigababyBody : ModProjectile
    {
        public override void SetDefaults()
        {
            projectile.width = 30;
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            int num214 = Main.projectileTexture[projectile.type].Height / Main.projFrames[projectile.type];
            int y6 = num214 * projectile.frame;
            Main.spriteBatch.Draw(texture2D13, projectile.Center - Main.screenPosition/* + new Vector2(0f, projectile.gfxOffY)*/, new Rectangle(0, 0, texture2D13.Width, texture2D13.Height),
                projectile.GetAlpha(Color.White), projectile.rotation, new Vector2(texture2D13.Width * 0.5f, texture2D13.Height * 0.5f), projectile.scale,
                projectile.spriteDirection == 1 ? SpriteEffects.None : SpriteEffects.FlipHorizontally, 0f);
            return false;
        }
        ...
        public override void AI()
        {
            ...
            if (byUUID >= 0 && Main.projectile[byUUID].active)
            {
                ...
                if (Main.projectile[byUUID].type != mod.ProjectileType("GigababyHead")) Main.projectile[byUUID].localAI[1] = projectile.whoAmI;
            }
            ...
            projectile.position = projectile.Center * 0.5f;
            projectile.scale = scaleFactor17;
            projectile.width = projectile.height = (int)(num1038 * projectile.scale);
            projectile.Center = projectile.position * 3;
            if (vector134 != Vector2.Zero) projectile.Center = value67 - Vector2.Normalize(vector134) * scaleFactor16 * scaleFactor17*1.81f;
        ...
        public override void Kill(int timeLeft)
        {
            ...
            if (player.slotsMinions + projectile.minionSlots > player.maxMinions && projectile.owner == Main.myPlayer)
            {
                ...
                if (byUUID != -1)
                {
                    ...
                    if (projectile1.type != mod.ProjectileType("GigababyHead")) projectile1.localAI[1] = projectile.localAI[1];
    ...
    public class GigababyTail : ModProjectile
    {
        public override void SetDefaults()
        {
            projectile.width = 30;
        ...
        public override void AI()
        {
            ...
            if (byUUID >= 0 && Main.projectile[byUUID].active)
            {
                ...
                Main.projectile[byUUID].localAI[0] = projectile.localAI[0] + 1f;
                if (Main.projectile[byUUID].type != mod.ProjectileType("GigababyHead")) Main.projectile[byUUID].localAI[1] = projectile.whoAmI;
                if (projectile.owner == player.whoAmI && Main.projectile[byUUID].type == mod.ProjectileType("GigababyHead"))
                {
            ...
            projectile.width = projectile.height = (int)(num1038 * projectile.scale);
            projectile.Center = projectile.position;
            if (vector134 != Vector2.Zero) projectile.Center = value67 - Vector2.Normalize(vector134) * 28;
            projectile.spriteDirection = vector134.X > 0f ? 1 : -1;
        }
    }
}
